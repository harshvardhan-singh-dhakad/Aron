...
import { mockListings, mockApplications } from '../../constants/mockData';
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
      if (user && listing) {
        checkIfApplied(listing.id, user.uid);
      }
  }, [user, listing]);
  ...
  const checkIfApplied = (listingId: string, userId: string) => {
     // Check against mock applications (or local state for new ones)
     const applied = mockApplications.some(
         app => app.listingId === listingId && app.applicantId === userId
     );
  ...
  const handleApply = async () => {
    ...
    setTimeout(() => {
        ...
        // In a real app we'd save this. For now just update local state visually
        setHasApplied(true);
        setApplying(false);
        ...
        <View className="p-5">
            <View className="flex-row justify-between items-start mb-4">
                ...
                <Text className="text-xs text-gray-400">
                    Posted {listing.createdAt instanceof Date ? listing.createdAt.toLocaleDateString() : 'recently'}
                </Text>
            </View>
            ...
            <View className="bg-gray-50 rounded-xl p-4 mb-24">
                 ...
                 <View className="flex-row justify-between py-2">
                     ...
                     <Text className="font-medium text-gray-900">#{listing.id.split('_')[1]}</Text>
                 </View>
            </View>
